...
//go:build benchmarkIncrementalRecall
// +build benchmarkIncrementalRecall
...
func TestIncrementalFilteredRecall(t *testing.T) {
...
func repairGraph(vectorIndex *hnsw, jobsForGraphRepairWorker [][]*vertex, IDsToFilter map[uint64]map[int]int,
	filterToIDs map[int]map[int][]uint64, IDsToVector map[uint64][]float32) {
	...
	for workerID, jobs := range jobsForGraphRepairWorker {
		...
		go func(workerID int, myJobs []*vertex) {
			...
			for _, node := range myJobs {
				nodeFilter, ok := IDsToFilter[node.id]
				if !ok {
					fmt.Printf("Couldn't get filter for node %d\n", node.id)
					continue
				}
				nodeAllowList := buildAllowList(nodeFilter, filterToIDs)
...
func evaluatePerformance(queryVectorsWithFilters []vecWithFilters, hnsw_efg bool, vectorIndex *hnsw, filterToIDs map[int]map[int][]uint64,
	truths []GroundTruths) {
	k := 100
