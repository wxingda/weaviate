...
func TestGetAnswer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			answer:  generateResponse{Error: &martianApiError{Message: "context deadline exceeded"}},
			timeout: time.Second,
		},
	}
	...
	t.Run("when X-Martian-BaseURL header is passed", func(t *testing.T) {
		c := New("apiKey", 5*time.Second, nullLogger())
		baseUrl := "https://api.endpoints.Martian.com"
		buildURL := c.getMartianUrl(context.Background(), baseUrl)
		assert.Equal(t, "https://api.endpoints.Martian.com/v1/chat/completions", buildURL)
	})
}
